...
def get_feed_dict(train_entity_pairs, start, end, train_user_set):
    ...
    feed_dict['pos_items'] = entity_pairs[:, 1]  # item id列表
...
if __name__ == '__main__':
    ...
    adj_mat_list, norm_mat_list, mean_mat_list = mat_list  # 邻接矩阵
    ...
    """cf data""" # 将user-item对转化为tensor [[userid,itemid]]
    train_cf_pairs = torch.LongTensor(np.array([[cf[0], cf[1]] for cf in train_cf], np.int32))
    test_cf_pairs = torch.LongTensor(np.array([[cf[0], cf[1]] for cf in test_cf], np.int32))

    """define model"""
    model = Recommender(n_params, args, graph, mean_mat_list[0]).to(device)    # 第0维是交互矩阵
